...
if authentication_status:
    ...
    # --- Item filter (depends on selected customers) ---
    ...
    # --- Status radio ---
    status_choice = st.sidebar.radio("Show rows with status:", ["All", "Reorder Soon", "OK"], index=0)
    ...
    if not filtered.empty:
        ...
        avg_days = (
            int(round(filtered["Days Until Out"].mean(skipna=True)))
            if not filtered["Days Until Out"].dropna().empty
            else 0
        )
    ...
    if not filtered.empty:
        ...
        fig_pie = px.pie(filtered, names="Status", title="Status Share", hole=0.35)
